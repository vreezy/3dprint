...
// Balken (zwischen box und box2)
balken_x = 0;                       // X-Mitte
balken_y = -25;   // Y-Mitte
balken_z = 45;   // Z-Mitte (liegt auf Oberkante box)
balken_w = box_w;                   // Breite (X)
balken_d = 10;                  // Tiefe  (Y)
balken_h = 10;                       // Höhe   (Z)
...
module turm_slit_at(ang, z_ctr) {
    total_h = fr_cum_h(fr_n) + fr_h_big + fr_h_small;
    rotate([0, 0, ang])
    translate([tower_r - win_depth/2, 0, z_ctr])
        cube([win_depth + 0.02, win_gap_w, total_h], center=true);
}
...
// D-förmiger Fuß (Zylinder + Rückseite) als dünne Scheibe bei Radius r
module turmdach_dscheibe(r) {
...
module turmdach(r1, r2, h1, h2) {
    base_r = tower_r;                 // Fuß bündig mit dem Turm → kein Überhang
    cham_h = (r1 - base_r) * 1.3;     // Fasenhöhe (~52° Unterseite, kein Support)
    color("red") {
        // Traufen-Fase: schräg vom Turmradius hoch zur Traufe (druckbar)
        hull() {
            turmdach_dscheibe(base_r );
            translate([0, 0, cham_h]) turmdach_dscheibe(r1);
        }
        // Hauptkegel: Traufe → Sechseck oben
        translate([0, 0, cham_h])
        hull() {
            turmdach_dscheibe(r1);
            translate([0, 0, h1 - cham_h])
...
// ── Kehlleiste: Viertelkreis-Profil ──────────────────────────────────────────
module kehlleiste_profil(r) {
    difference() {
        square(r * 2);
        circle(r);
    }
...
// ── Balken (45°-Fase als Stützersatz für den Überhang) ───────────────────────
// Gerader Keil statt rundem Zylinderschnitt: die Unterseite läuft mit
// konstanter Neigung von der box-Vorderkante hoch zur box2-Vorderkante,
// dadurch nirgends flacher als die Druckbarkeitsgrenze (kein Support nötig).
module balken(x, y, z, w, d, h) {
    yb = y + d/2;   // box-Seite  (hinten, +Y)
    yf = y - d/2;   // box2-Seite (vorne,  -Y, Überhangkante)
    zb = z - h/2;   // unten
    zt = z + h/2;   // oben (Unterkante box2)
    eps = 0.01;
    color("lightblue")
    hull() {
        translate([x, yb, zb]) cube([w, eps, eps], center=true); // Fuß an box
        translate([x, yb, zt]) cube([w, eps, eps], center=true); // hintere Oberkante
        translate([x, yf, zt]) cube([w, eps, eps], center=true); // vordere Oberkante (trägt box2)
    }
}
...
translate([0, box_y, 0]) {
    difference() {
        ...
        // Schießschacht vorne (bündig, 2 tief)
        translate([0, -box_d/2 + win_depth/2, win_z])
            cube([win_gap_w, win_depth + 0.02, win_height], center=true);
    }
    arch_verzierung();
}

// Balken Rundung zwischen box und box2
balken(balken_x, balken_y, balken_z, balken_w, balken_d, balken_h);
